      ...
      <div className="relative px-6 py-7 md:px-8 md:py-8">
        <div className="flex items-start justify-between gap-6 flex-wrap">
          <div className="flex-1 min-w-[260px]">
            ...
            <h1 className="text-[24px] md:text-[28px] font-bold tracking-tight leading-tight">{title}</h1>
            {description && <p className="text-[13px] md:text-[14px] mt-1.5 opacity-85 max-w-xl leading-relaxed">{description}</p>}
            {chips && chips.length > 0 && (
              <div className="flex flex-wrap items-center gap-2 mt-5">
                {chips.map((chip, i) => (
                  <span key={i} className="inline-flex items-center gap-2 px-3 py-1.5 bg-white/15 backdrop-blur-sm rounded-full text-[12px] font-medium ring-1 ring-white/20 whitespace-nowrap">
                    {chip}
                  </span>
                ))}
              </div>
